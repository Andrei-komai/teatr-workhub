...
export function ScheduleScreen({ title, description, events, entries, regularAbsences, participantNames, currentProfileId, currentRole, canManage, onBack, onSaveEvent, onDeleteEvent, onSaveEntry, onDeleteEntry, onSaveRegularAbsence }: { title: string; description: string; events: CalendarEvent[]; entries: ScheduleEntry[]; regularAbsences: ScheduleRegularAbsence[]; participantNames: Record<string, string>; currentProfileId: string | null; currentRole: string; canManage: boolean; onBack: () => void; onSaveEvent: (input: CalendarEventInput, initial: CalendarEvent | null) => Promise<boolean>; onDeleteEvent: (item: CalendarEvent) => Promise<void>; onSaveEntry: (input: ScheduleEntryInput, initial: ScheduleEntry | null) => Promise<boolean>; onDeleteEntry: (item: ScheduleEntry) => Promise<void>; onSaveRegularAbsence: (seriesId: string, reason: string) => Promise<boolean> }) {
  const [view, setView] = useState<CalendarView>('month')
  const [cursor, setCursor] = useState(() => new Date())
