...
export function ScheduleScreen({ title, description, events, canManage, onBack, onSave, onDelete }: { title: string; description: string; events: CalendarEvent[]; canManage: boolean; onBack: () => void; onSave: (input: CalendarEventInput, initial: CalendarEvent | null) => Promise<boolean>; onDelete: (item: CalendarEvent) => Promise<void> }) {
  const [editor, setEditor] = useState<{ initial: CalendarEvent | null; date: string } | null>(null)
  const synchronized = events.slice().sort(eventSort)
  return <main><section className="work-header compact"><button className="icon-button inverse" type="button" aria-label="Назад" onClick={onBack}>←</button><div><h1>{title}</h1><p>{description}</p></div>{canManage && <button className="button inverse-button" type="button" onClick={() => setEditor({ initial: null, date: dateKey(new Date()) })}>＋ Добавить</button>}</section><section className="schedule-sync-note"><span>↻</span><div><b>Синхронизировано с календарём репертуара</b><p>Все события появляются здесь автоматически. Изменение записи обновит оба раздела.</p></div></section><section className="calendar-list schedule-list"><EventGroups events={synchronized} canManage={canManage} emptyText="События пока не добавлены" onEdit={(item) => setEditor({ initial: item, date: item.eventDate })} onDelete={onDelete} /></section>{editor && <EventEditor initial={editor.initial} defaultDate={editor.date} onClose={() => setEditor(null)} onSave={onSave} />}</main>
}
