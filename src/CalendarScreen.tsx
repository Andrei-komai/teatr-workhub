...
export type ScheduleAbsence = { entryId: string; profileId: string; reason: string }
...
function OwnAbsenceEditor({ current, onClose, onSave }: { current: string; onClose: () => void; onSave: (reason: string) => Promise<boolean> }) {
  const [saving, setSaving] = useState(false)
  async function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault()
    const reason = String(new FormData(event.currentTarget).get('reason') ?? '').trim()
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={onClose}><form className="calendar-event-editor regular-absence-editor" role="dialog" aria-modal="true" aria-labelledby="own-absence-title" onMouseDown={(event) => event.stopPropagation()} onSubmit={submit}>
    <button className="modal-close" type="button" aria-label="Закрыть" onClick={onClose}>×</button>
    <p className="eyebrow">Личная отметка</p><h2 id="own-absence-title">Отсутствие на эту дату</h2>
    <p>Запись относится только к вам. Другой пользователь не сможет изменить или удалить её.</p>
    <label><span>Краткая причина</span><textarea name="reason" defaultValue={current} rows={3} maxLength={500} placeholder="Например: работа до 19:00" autoFocus /></label>
    <div className="calendar-form-actions"><button className="button" type="button" onClick={onClose}>Отмена</button>{current && <button className="button danger-button" type="button" disabled={saving} onClick={async () => { if (!window.confirm('Убрать ваше отсутствие на эту дату?')) return; setSaving(true); const saved = await onSave(''); setSaving(false); if (saved) onClose() }}>Убрать отсутствие</button>}<button className="button button-solid" type="submit" disabled={saving}>{saving ? 'Сохраняю…' : 'Сохранить'}</button></div>
  </form></div>
}
...
function ScheduleEntryCard({ item, canManage, absences, regularAbsences, participantNames, canSetAbsence, ownProfileId, onEdit, onDelete, onAbsence, onRegularAbsence }: { item: ScheduleEntry; canManage: boolean; absences: ScheduleAbsence[]; regularAbsences: ScheduleRegularAbsence[]; participantNames: Record<string, string>; canSetAbsence: boolean; ownProfileId: string | null; onEdit: (item: ScheduleEntry) => void; onDelete: (item: ScheduleEntry) => void; onAbsence: (entryId: string, current: string) => void; onRegularAbsence: (seriesId: string, current: string) => void }) {
  const entryAbsences = absences.filter((absence) => absence.entryId === item.id)
  const seriesAbsences = item.seriesId ? regularAbsences.filter((absence) => absence.seriesId === item.seriesId) : []
  const ownAbsence = entryAbsences.find((absence) => absence.profileId === ownProfileId)?.reason ?? ''
  const title = item.className || 'Занятие'
  const ownRegularAbsence = seriesAbsences.find((absence) => absence.profileId === ownProfileId)?.reason ?? ''
  return <article className="schedule-entry-card"><div className="calendar-event-time"><b>{formatTime(item.startTime)}</b>{item.endTime && <span>— {formatTime(item.endTime)}</span>}</div><div className="schedule-entry-content"><div className="schedule-entry-labels"><span className="event-type schedule">Занятие</span>{item.seriesId && <span className="event-type recurring">Регулярное</span>}</div><h3>{title}</h3><dl><div><dt>Педагог</dt><dd>{item.teacher || 'Пока не указан'}</dd></div>{item.topic && <div><dt>О чём</dt><dd>{item.topic}</dd></div>}{item.absence && <div className="schedule-absence"><dt>На эту дату — старые записи</dt><dd>{item.absence}</dd></div>}{entryAbsences.length > 0 && <div className="schedule-absence regular"><dt>На эту дату</dt><dd>{entryAbsences.map((absence) => <span key={absence.profileId}><b>{participantNames[absence.profileId] ?? 'Участник'}</b>{absence.reason ? ` — ${absence.reason}` : ''}</span>)}</dd></div>}{seriesAbsences.length > 0 && <div className="schedule-absence regular"><dt>Регулярно</dt><dd>{seriesAbsences.map((absence) => <span key={absence.profileId}><b>{participantNames[absence.profileId] ?? 'Участник'}</b>{absence.reason ? ` — ${absence.reason}` : ''}</span>)}</dd></div>}</dl>{canSetAbsence && <div className="schedule-own-absence-actions"><button className="button schedule-absence-button" type="button" onClick={() => onAbsence(item.id, ownAbsence)}>{ownAbsence ? 'Изменить моё отсутствие' : 'Указать моё отсутствие'}</button>{item.seriesId && <button className="button schedule-absence-button" type="button" onClick={() => onRegularAbsence(item.seriesId!, ownRegularAbsence)}>{ownRegularAbsence ? 'Изменить регулярное отсутствие' : 'Указать регулярное отсутствие'}</button>}</div>}</div>{canManage && <div className="calendar-event-actions"><button className="icon-button" type="button" aria-label={`Редактировать ${title}`} onClick={() => onEdit(item)}>✎</button><button className="icon-button danger" type="button" aria-label={`Удалить ${title}`} onClick={() => onDelete(item)}>×</button></div>}</article>
}

function ScheduleGroups({ entries, events, canManage, absences, regularAbsences, participantNames, canSetAbsence, ownProfileId, emptyText, onEditEntry, onDeleteEntry, onEditEvent, onDeleteEvent, onAbsence, onRegularAbsence }: { entries: ScheduleEntry[]; events: CalendarEvent[]; canManage: boolean; absences: ScheduleAbsence[]; regularAbsences: ScheduleRegularAbsence[]; participantNames: Record<string, string>; canSetAbsence: boolean; ownProfileId: string | null; emptyText: string; onEditEntry: (item: ScheduleEntry) => void; onDeleteEntry: (item: ScheduleEntry) => void; onEditEvent: (item: CalendarEvent) => void; onDeleteEvent: (item: CalendarEvent) => void; onAbsence: (entryId: string, current: string) => void; onRegularAbsence: (seriesId: string, current: string) => void }) {
  ...
  return <div className="calendar-event-groups">{days.map((day) => { const dayEntries = entries.filter((item) => item.eventDate === day).sort(scheduleSort); const dayEvents = events.filter((item) => item.eventDate === day).sort(eventSort); return <section className="calendar-day-group" key={day}><h2>{formatDay(day)}</h2>{[...dayEntries.map((item) => ({ kind: 'schedule' as const, time: item.startTime, item })), ...dayEvents.map((item) => ({ kind: 'calendar' as const, time: item.startTime, item }))].sort((left, right) => left.time.localeCompare(right.time)).map((row) => row.kind === 'schedule' ? <ScheduleEntryCard key={`schedule-${row.item.id}`} item={row.item as ScheduleEntry} canManage={canManage} absences={absences} regularAbsences={regularAbsences} participantNames={participantNames} canSetAbsence={canSetAbsence} ownProfileId={ownProfileId} onEdit={onEditEntry} onDelete={onDeleteEntry} onAbsence={onAbsence} onRegularAbsence={onRegularAbsence} /> : <EventCard key={`calendar-${row.item.id}`} item={row.item as CalendarEvent} canManage={canManage} onEdit={onEditEvent} onDelete={onDeleteEvent} />)}</section> })}</div>
}

export function ScheduleScreen({ title, description, events, entries, absences, regularAbsences, participantNames, currentProfileId, canManage, onBack, onSaveEvent, onDeleteEvent, onSaveEntry, onDeleteEntry, onSaveAbsence, onSaveRegularAbsence }: { title: string; description: string; events: CalendarEvent[]; entries: ScheduleEntry[]; absences: ScheduleAbsence[]; regularAbsences: ScheduleRegularAbsence[]; participantNames: Record<string, string>; currentProfileId: string | null; canManage: boolean; onBack: () => void; onSaveEvent: (input: CalendarEventInput, initial: CalendarEvent | null) => Promise<boolean>; onDeleteEvent: (item: CalendarEvent) => Promise<void>; onSaveEntry: (input: ScheduleEntryInput, initial: ScheduleEntry | null) => Promise<boolean>; onDeleteEntry: (item: ScheduleEntry) => Promise<void>; onSaveAbsence: (entryId: string, reason: string) => Promise<boolean>; onSaveRegularAbsence: (seriesId: string, reason: string) => Promise<boolean> }) {
  const [view, setView] = useState<CalendarView>('month')
  const [cursor, setCursor] = useState(() => new Date())
  ...
  const [ownAbsenceEditor, setOwnAbsenceEditor] = useState<{ entryId: string; current: string } | null>(null)
  ...
  return <main><section className="work-header compact calendar-header"><button className="icon-button inverse" type="button" aria-label="Назад" onClick={onBack}>←</button><div><h1>{title}</h1><p>{description}</p></div>{canManage && <button className="button inverse-button" type="button" onClick={() => setEntryEditor({ initial: null, date: dateKey(cursor) })}>＋ Добавить</button>}</section>
    <section className="calendar-shell schedule-calendar-shell"><div className="calendar-toolbar"><div className="calendar-view-switch" aria-label="Вид календаря расписания">{(['week', 'month', 'year'] as CalendarView[]).map((item) => <button className={view === item ? 'active' : ''} type="button" onClick={() => setView(item)} key={item}>{item === 'week' ? 'Неделя' : item === 'month' ? 'Месяц' : 'Год'}</button>)}</div><div className="calendar-period"><button className="icon-button" type="button" aria-label="Предыдущий период" onClick={() => move(-1)}>←</button><button className="button calendar-today" type="button" onClick={() => setCursor(new Date())}>Сегодня</button><button className="icon-button" type="button" aria-label="Следующий период" onClick={() => move(1)}>→</button></div><h2>{rangeTitle(view, cursor)}</h2></div>
      ...
      <section className="calendar-list schedule-list"><div className="calendar-list-heading"><p className="eyebrow">Заполненные дни</p><h2>{rangeTitle(view, cursor)}</h2></div><ScheduleGroups entries={visibleEntries} events={visibleEvents} canManage={canManage} absences={absences} regularAbsences={regularAbsences} participantNames={participantNames} canSetAbsence={Boolean(currentProfileId)} ownProfileId={currentProfileId} emptyText={emptyText} onEditEntry={(item) => setEntryEditor({ initial: item, date: item.eventDate })} onDeleteEntry={onDeleteEntry} onEditEvent={setEventEditor} onDeleteEvent={onDeleteEvent} onAbsence={(entryId, current) => setOwnAbsenceEditor({ entryId, current })} onRegularAbsence={(seriesId, current) => setAbsenceEditor({ seriesId, current })} /></section>
    </section>{entryEditor && <ScheduleEntryEditor initial={entryEditor.initial} defaultDate={entryEditor.date} onClose={() => setEntryEditor(null)} onSave={onSaveEntry} />}{eventEditor && <EventEditor initial={eventEditor} defaultDate={eventEditor.eventDate} onClose={() => setEventEditor(null)} onSave={onSaveEvent} />}{ownAbsenceEditor && <OwnAbsenceEditor current={ownAbsenceEditor.current} onClose={() => setOwnAbsenceEditor(null)} onSave={(reason) => onSaveAbsence(ownAbsenceEditor.entryId, reason)} />}{absenceEditor && <RegularAbsenceEditor current={absenceEditor.current} onClose={() => setAbsenceEditor(null)} onSave={(reason) => onSaveRegularAbsence(absenceEditor.seriesId, reason)} />}
  </main>
}
