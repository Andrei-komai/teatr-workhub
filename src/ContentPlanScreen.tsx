...
function PlanRow({ kind, initial, defaultDate, onSave, onDelete, onDiscardDraft, onSaved }: { kind: ContentPlanKind; initial?: ContentPlanItem; defaultDate: string; onSave: (input: ContentPlanInput, initial: ContentPlanItem | null) => Promise<boolean>; onDelete?: (item: ContentPlanItem) => void; onDiscardDraft?: () => void; onSaved?: () => void }) {
  ...
  return <form className={`content-plan-row ${kind}`} onSubmit={submit}>
    <label className="content-plan-date"><span>Дата</span><input name="contentDate" type="date" value={date} onChange={(event) => setDate(event.target.value)} required /></label>
    ...
    <div className="content-plan-row-actions"><button className="button button-solid" type="submit" disabled={saving}>{saving ? 'Сохраняю…' : initial ? 'Сохранить' : 'Добавить'}</button>{initial && onDelete && <button className="icon-button danger" type="button" aria-label="Удалить запись" onClick={() => onDelete(initial)}>×</button>}{!initial && onDiscardDraft && <button className="button button-quiet content-plan-discard" type="button" onClick={onDiscardDraft}>Убрать</button>}</div>
    {error && <p className="content-plan-row-error" role="alert">{error}</p>}
  </form>
}

function DraftRows({ kind, defaultDate, onSave }: { kind: ContentPlanKind; defaultDate: string; onSave: (input: ContentPlanInput, initial: ContentPlanItem | null) => Promise<boolean> }) {
  ...
  function discardDraft(id: string) {
    if (window.confirm('Убрать эту пустую строку?')) setDrafts((current) => current.filter((draft) => draft.id !== id))
  }
  return <div className="content-plan-drafts">{drafts.map((draft, index) => <PlanRow kind={kind} defaultDate={draft.date} onSave={onSave} onDiscardDraft={index > 0 ? () => discardDraft(draft.id) : undefined} onSaved={() => replaceSaved(draft.id)} key={draft.id} />)}<button className="button content-plan-add-row" type="button" onClick={() => setDrafts((current) => [...current, { id: crypto.randomUUID(), date: defaultDate }])}>＋ Добавить ещё строку</button></div>
}

function ReadOnlyPlanRow({ item }: { item: ContentPlanItem }) {
  return <article className={`content-plan-row ${item.kind} content-plan-readonly-row`}><div className="content-plan-date"><span>Дата</span><b>{formatDay(item.contentDate)}</b></div><div className="content-plan-description"><span>{item.kind === 'current' ? 'Что публикуем' : 'Смыслы'}</span><p>{item.description}</p></div>{item.kind === 'development' && <div><span>Формат</span><p>{item.format || 'Не указан'}</p></div>}<div><span>Ответственный</span><p>{item.responsible || 'Не указан'}</p></div><div className="content-plan-resource"><span>Ссылка или файлы</span>{item.link && <a className="content-plan-link" href={item.link} target="_blank" rel="noreferrer">Открыть ссылку ↗</a>}<AttachmentList files={item.attachments} /></div></article>
}

export function ContentPlanScreen({ title, description, entries, canManage, onBack, onSave, onDelete }: { title: string; description: string; entries: ContentPlanItem[]; canManage: boolean; onBack: () => void; onSave: (input: ContentPlanInput, initial: ContentPlanItem | null) => Promise<boolean>; onDelete: (item: ContentPlanItem) => void }) {
  ...
  return <main><section className="work-header compact"><button className="icon-button inverse" type="button" aria-label="Назад" onClick={onBack}>←</button><div><h1>{title}</h1><p>{description}</p></div></section>
    <section className="content-plan-shell"><div className="settings-tabs content-plan-tabs" role="tablist" aria-label="Разделы контент-плана"><button type="button" role="tab" aria-selected={tab === 'current'} className={tab === 'current' ? 'active' : ''} onClick={() => setTab('current')}>Текущий контент-план</button><button type="button" role="tab" aria-selected={tab === 'development'} className={tab === 'development' ? 'active' : ''} onClick={() => setTab('development')}>Разработка контента</button></div>
      {tab === 'current' ? <><div className="content-plan-toolbar"><div className="calendar-view-switch"><button className={view === 'week' ? 'active' : ''} type="button" onClick={() => setView('week')}>Неделя</button><button className={view === 'month' ? 'active' : ''} type="button" onClick={() => setView('month')}>Месяц</button></div><div className="calendar-period"><button className="icon-button" type="button" aria-label="Предыдущий период" onClick={() => moveCurrent(-1)}>←</button><button className="button calendar-today" type="button" onClick={() => { const today = new Date(); setCursor(today); setSelectedDate(dateKey(today)) }}>Сегодня</button><button className="icon-button" type="button" aria-label="Следующий период" onClick={() => moveCurrent(1)}>→</button></div><h2>{rangeTitle(view, cursor)}</h2></div>
        ...
        <section className="content-plan-table-section"><div className="content-plan-section-heading"><p className="eyebrow">Текущий контент-план</p><h2>{selectedDate ? `Выбрано: ${formatDay(selectedDate)}` : rangeTitle(view, cursor)}</h2></div>{canManage && <DraftRows kind="current" defaultDate={selectedDate} onSave={onSave} />}{currentEntries.length > 0 && <div className="content-plan-saved"><h3>Заполненные строки</h3>{currentEntries.map((item) => canManage ? <PlanRow kind="current" initial={item} defaultDate={item.contentDate} onSave={onSave} onDelete={onDelete} key={item.id} /> : <ReadOnlyPlanRow item={item} key={item.id} />)}</div>}</section></> : <><div className="content-development-toolbar"><div><p className="eyebrow">Разработка контента</p><h2>{monthTitle(developmentCursor)}</h2></div><div className="calendar-period"><button className="icon-button" type="button" aria-label="Предыдущий месяц" onClick={() => setDevelopmentCursor((current) => addMonths(current, -1))}>←</button><button className="button calendar-today" type="button" onClick={() => setDevelopmentCursor(new Date())}>Текущий месяц</button><button className="icon-button" type="button" aria-label="Следующий месяц" onClick={() => setDevelopmentCursor((current) => addMonths(current, 1))}>→</button></div></div>
        <section className="content-plan-table-section development">{canManage && <DraftRows kind="development" defaultDate={developmentStart} onSave={onSave} />}{developmentEntries.length > 0 && <div className="content-plan-saved"><h3>Заполненные строки</h3>{developmentEntries.map((item) => canManage ? <PlanRow kind="development" initial={item} defaultDate={item.contentDate} onSave={onSave} onDelete={onDelete} key={item.id} /> : <ReadOnlyPlanRow item={item} key={item.id} />)}</div>}</section></>}
    </section>
  </main>
}
